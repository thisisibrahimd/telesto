            ...
            filters: {
              ...
              externalAuth: {
                ...
                backendRef: {
                  ...
                  withKind(kind): { externalAuth+: { backendRef+: { kind: kind } } },
                  '#withName':: d.fn(help='"Name is the name of the referent."', args=[d.arg(name='name', type=d.T.string)]),
                  withName(name): { externalAuth+: { backendRef+: { name: name } } },
                  ...
                  withNamespace(namespace): { externalAuth+: { backendRef+: { namespace: namespace } } },
                  ...
                  withPort(port): { externalAuth+: { backendRef+: { port: port } } },
                },
                '#forwardBody':: d.obj(help='"ForwardBody controls if requests to the authorization server should include\\nthe body of the client request; and if so, how big that body is allowed\\nto be.\\n\\nIt is expected that implementations will buffer the request body up to\\n`forwardBody.maxSize` bytes. Bodies over that size must be rejected with a\\n4xx series error (413 or 403 are common examples), and fail processing\\nof the filter.\\n\\nIf unset, or `forwardBody.maxSize` is set to `0`, then the body will not\\nbe forwarded.\\n\\nFeature Name: HTTPRouteExternalAuthForwardBody"'),
                forwardBody: {
                  '#withMaxSize':: d.fn(help='"MaxSize specifies how large in bytes the largest body that will be buffered\\nand sent to the authorization server. If the body size is larger than\\n`maxSize`, then the body sent to the authorization server must be\\ntruncated to `maxSize` bytes.\\n\\nExperimental note: This behavior needs to be checked against\\nvarious dataplanes; it may need to be changed.\\nSee https://github.com/kubernetes-sigs/gateway-api/pull/4001#discussion_r2291405746\\nfor more.\\n\\nIf 0, the body will not be sent to the authorization server."', args=[d.arg(name='maxSize', type=d.T.integer)]),
                  withMaxSize(maxSize): { externalAuth+: { forwardBody+: { maxSize: maxSize } } },
                },
                '#grpc':: d.obj(help='"GRPCAuthConfig contains configuration for communication with ext_authz\\nprotocol-speaking backends.\\n\\nIf unset, implementations must assume the default behavior for each\\nincluded field is intended."'),
                grpc: {
                  '#withAllowedHeaders':: d.fn(help='"AllowedRequestHeaders specifies what headers from the client request\\nwill be sent to the authorization server.\\n\\nIf this list is empty, then all headers must be sent.\\n\\nIf the list has entries, only those entries must be sent."', args=[d.arg(name='allowedHeaders', type=d.T.array)]),
                  withAllowedHeaders(allowedHeaders): { externalAuth+: { grpc+: { allowedHeaders: if std.isArray(v=allowedHeaders) then allowedHeaders else [allowedHeaders] } } },
                  '#withAllowedHeadersMixin':: d.fn(help='"AllowedRequestHeaders specifies what headers from the client request\\nwill be sent to the authorization server.\\n\\nIf this list is empty, then all headers must be sent.\\n\\nIf the list has entries, only those entries must be sent."\n\n**Note:** This function appends passed data to existing values', args=[d.arg(name='allowedHeaders', type=d.T.array)]),
                  withAllowedHeadersMixin(allowedHeaders): { externalAuth+: { grpc+: { allowedHeaders+: if std.isArray(v=allowedHeaders) then allowedHeaders else [allowedHeaders] } } },
                },
                '#http':: d.obj(help='"HTTPAuthConfig contains configuration for communication with HTTP-speaking\\nbackends.\\n\\nIf unset, implementations must assume the default behavior for each\\nincluded field is intended."'),
                http: {
                  '#withAllowedHeaders':: d.fn(help='"AllowedRequestHeaders specifies what additional headers from the client request\\nwill be sent to the authorization server.\\n\\nThe following headers must always be sent to the authorization server,\\nregardless of this setting:\\n\\n* `Host`\\n* `Method`\\n* `Path`\\n* `Content-Length`\\n* `Authorization`\\n\\nIf this list is empty, then only those headers must be sent.\\n\\nNote that `Content-Length` has a special behavior, in that the length\\nsent must be correct for the actual request to the external authorization\\nserver - that is, it must reflect the actual number of bytes sent in the\\nbody of the request to the authorization server.\\n\\nSo if the `forwardBody` stanza is unset, or `forwardBody.maxSize` is set\\nto `0`, then `Content-Length` must be `0`. If `forwardBody.maxSize` is set\\nto anything other than `0`, then the `Content-Length` of the authorization\\nrequest must be set to the actual number of bytes forwarded."', args=[d.arg(name='allowedHeaders', type=d.T.array)]),
                  withAllowedHeaders(allowedHeaders): { externalAuth+: { http+: { allowedHeaders: if std.isArray(v=allowedHeaders) then allowedHeaders else [allowedHeaders] } } },
                  '#withAllowedHeadersMixin':: d.fn(help='"AllowedRequestHeaders specifies what additional headers from the client request\\nwill be sent to the authorization server.\\n\\nThe following headers must always be sent to the authorization server,\\nregardless of this setting:\\n\\n* `Host`\\n* `Method`\\n* `Path`\\n* `Content-Length`\\n* `Authorization`\\n\\nIf this list is empty, then only those headers must be sent.\\n\\nNote that `Content-Length` has a special behavior, in that the length\\nsent must be correct for the actual request to the external authorization\\nserver - that is, it must reflect the actual number of bytes sent in the\\nbody of the request to the authorization server.\\n\\nSo if the `forwardBody` stanza is unset, or `forwardBody.maxSize` is set\\nto `0`, then `Content-Length` must be `0`. If `forwardBody.maxSize` is set\\nto anything other than `0`, then the `Content-Length` of the authorization\\nrequest must be set to the actual number of bytes forwarded."\n\n**Note:** This function appends passed data to existing values', args=[d.arg(name='allowedHeaders', type=d.T.array)]),
                  withAllowedHeadersMixin(allowedHeaders): { externalAuth+: { http+: { allowedHeaders+: if std.isArray(v=allowedHeaders) then allowedHeaders else [allowedHeaders] } } },
                  '#withAllowedResponseHeaders':: d.fn(help='"AllowedResponseHeaders specifies what headers from the authorization response\\nwill be copied into the request to the backend.\\n\\nIf this list is empty, then all headers from the authorization server\\nexcept Authority or Host must be copied."', args=[d.arg(name='allowedResponseHeaders', type=d.T.array)]),
                  withAllowedResponseHeaders(allowedResponseHeaders): { externalAuth+: { http+: { allowedResponseHeaders: if std.isArray(v=allowedResponseHeaders) then allowedResponseHeaders else [allowedResponseHeaders] } } },
                  '#withAllowedResponseHeadersMixin':: d.fn(help='"AllowedResponseHeaders specifies what headers from the authorization response\\nwill be copied into the request to the backend.\\n\\nIf this list is empty, then all headers from the authorization server\\nexcept Authority or Host must be copied."\n\n**Note:** This function appends passed data to existing values', args=[d.arg(name='allowedResponseHeaders', type=d.T.array)]),
                  withAllowedResponseHeadersMixin(allowedResponseHeaders): { externalAuth+: { http+: { allowedResponseHeaders+: if std.isArray(v=allowedResponseHeaders) then allowedResponseHeaders else [allowedResponseHeaders] } } },
                  '#withPath':: d.fn(help='"Path sets the prefix that paths from the client request will have added\\nwhen forwarded to the authorization server.\\n\\nWhen empty or unspecified, no prefix is added.\\n\\nValid values are the same as the \\"value\\" regex for path values in the `match`\\nstanza, and the validation regex will screen out invalid paths in the same way.\\nEven with the validation, implementations MUST sanitize this input before using it\\ndirectly."', args=[d.arg(name='path', type=d.T.string)]),
                  withPath(path): { externalAuth+: { http+: { path: path } } },
                },
                '#withProtocol':: d.fn(help='"ExternalAuthProtocol describes which protocol to use when communicating with an\\next_authz authorization server.\\n\\nWhen this is set to GRPC, each backend must use the Envoy ext_authz protocol\\non the port specified in `backendRefs`. Requests and responses are defined\\nin the protobufs explained at:\\nhttps://www.envoyproxy.io/docs/envoy/latest/api-v3/service/auth/v3/external_auth.proto\\n\\nWhen this is set to HTTP, each backend must respond with a `200` status\\ncode in on a successful authorization. Any other code is considered\\nan authorization failure.\\n\\nFeature Names:\\nGRPC Support - HTTPRouteExternalAuthGRPC\\nHTTP Support - HTTPRouteExternalAuthHTTP"', args=[d.arg(name='protocol', type=d.T.string)]),
                withProtocol(protocol): { externalAuth+: { protocol: protocol } },
              },
